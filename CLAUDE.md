...
| 1 — MVP daily routine | 🟡 ~99.9% | M1+M2+M3+M4 code-complete; rounds 12-25 polish layer (see CHANGELOG history). Round 27 added WS-A AI itinerary wizard + WS-B birthdays/important-days on Today + Settings IA collapse (29→7 pages) + destination autocomplete + map preview + home-location auto-detect + travel-advisory reorder + chip-row redesign. Round 28 = lint debt cleanup (65 → 0 errors, refactor only, no behavior change). Round 29 = CI fix (Swift 6 sendability via `@preconcurrency import UserNotifications`) + Wizard v2 (persist generated text, 3 deep-link buttons Claude/ChatGPT/Perplexity, clear-saved) + App Store prep docs (PRIVACY.md + LISTING.md in EN/ES/FR + T-277). Round 30 = drift cleanup + L009 (Swift 6 host-target language-mode skew local↔CI) + L010 (USB `._*` resource forks inflate `find` counts) promoted to LESSONS.md + `scripts/check-counts.sh` guard. Remaining: real-device validation + paid Apple Developer Program. |
...
> Source of truth for phase progress — must match `memory/project_status.md` and `ROADMAP.md § Phases`. Last synced: round 30.
