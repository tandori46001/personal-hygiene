...
| 1 — MVP daily routine | 🟡 ~99.95% | M1+M2+M3+M4 code-complete; rounds 12-21 polish layer (per-block followup, pause, theme, mute, bedtime auto-mute, quiet hours, dose history + adherence + skip-dose, stale-day banner, conflict detector + overlap visualizer, undo preset, snooze-30, mood quick-log + good-days caption + disclosure + week strip + 30-day Charts trend + emoji filter + weekly goal + localized CSV, tomorrow disclosure, about-build footer, now-marker tap-to-scroll, reset-day undo, block-title history, renumber start times, refresh-trace toast, block CSV import, ⌘D iPad shortcut, ad-hoc Focus toggle). Remaining: real-device validation + paid Apple Developer Program. |
| 2 — Apple Watch | ✅ ~98% | Round 19 added complication line-3, theme/pause mirrors, snooze-30 + skip-dose actions. Round 21 added Hydration glance + Mood quick-log surfaces, complication pause badge, mark-done undo capsule (3s). Watch on `1d70b50` (round-19 deploy); round-21 redeploy pending watch availability. |
| 3 — Secondary modules | ✅ Done | M5 hydration, M6 housekeeping (+ streak + auto-snooze helper), M7 birthdays (+ relationship/idea stores + gift CSV exporter + lead-time global default), M8 deep focus (+ right-now + conflict + ad-hoc toggle + category mute mirror to watch). |
...
> Source of truth for phase progress — must match `memory/project_status.md` and `ROADMAP.md § Phases`. Last synced: round 21.
