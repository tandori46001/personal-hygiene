...
> **Round 30 (2026-04-29, drift cleanup):** Batch A drift commit + L009 + L010 promoted from `feedback_repo_quirks.md` to LESSONS.md + `scripts/check-counts.sh` added (L010 guard — canonical-count audit that always excludes `._*` AppleDouble resource forks). Reconciled MVP % across CLAUDE.md §8 + ROADMAP (was 99.99% / 99.5% — picked 99.9%). Real `@Model` count = 14 (memory said 16-17), all-test files = 205 (memory said 224). iPhone live `eae6d9b` per build-time `CommitSHA.txt` stamp (was wrongly recorded as `8ec08c4` in round-29 wrap; the file is auto-generated by `deploy-iphone.sh` and was last stamped today at 16:12 — verified `eae6d9b` is HEAD~1 = the SWIFT_VERSION=5 revert, before the Int→Double test-only fix). Tests **940** unchanged (no test changes in round 30). i18n **997 × 3** unchanged. Lessons **10** (+2). No behavior change.
