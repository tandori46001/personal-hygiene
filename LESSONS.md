...
## L013 — Wiring `CODE_SIGN_ENTITLEMENTS` requesting iCloud / App Group / HealthKit BEFORE the matching App ID + container exists on developer.apple.com breaks SwiftData tests

**Trigger pattern.** A target's `.entitlements` file declares `com.apple.developer.icloud-services: [CloudKit]` + `com.apple.developer.icloud-container-identifiers: [iCloud.com.foo.bar]` + `com.apple.security.application-groups: [group.com.foo.bar]`. The matching iCloud Container, App Group, or App ID does NOT yet exist on developer.apple.com. The target's `settings.base.CODE_SIGN_ENTITLEMENTS` is wired to the .entitlements file in `App/project.yml`. Build succeeds, but at runtime SwiftData's `ModelContainer.init` throws `SwiftDataError._Error.loadIssueModelContainer` for **every** test that builds a `ModelContainer` — even an in-memory, no-CloudKit one — because SwiftData reads the host bundle's entitlements at container-init time and rejects setup when declared identifiers can't be validated against developer-portal resources.

**Symptom.** `./scripts/check-tests.sh` after `xcodegen generate` reports `13 tests failed (13 unexpected)` — all in test classes that exercise `ModelContainer` (e.g. `TripDetailViewModelRound12Tests`, `TripDetailViewModelRound13Tests`, `TripDetailViewModelRound14Tests`). Each failure surfaces `caught error: "SwiftDataError(_error: SwiftData.SwiftDataError._Error.loadIssueModelContainer, _explanation: nil)"` followed by `Test skipped: threw error "The operation couldn't be completed. (SwiftData.SwiftDataError error 1.)"`. Tests that DON'T touch `ModelContainer` (pure-function tests, advisory-service tests, view-model tests with mocked stores) keep passing — the failure surface is exactly the SwiftData-using subset.

The kicker: tests passed minutes earlier, before the entitlements wiring commit. The diff that broke them is `App/project.yml` adding a single `CODE_SIGN_ENTITLEMENTS:` line per target. Visually a one-line change, semantically a "promise the simulator's iCloud daemon you're going to talk to a container that doesn't exist."

**Root cause.** Apple's signing + entitlements model treats CloudKit container identifiers and App Group identifiers as **declarative claims** that the platform validates against developer-portal records. In simulator, the platform layer can't reach the developer portal — but it CAN check that the identifier is well-formed and that the running bundle's signing identity matches the team that owns the App ID. With `CODE_SIGN_STYLE: Automatic` + a paid team, Xcode tries to provision the entitlements automatically; when the App ID + container don't exist on the portal yet, that auto-provision fails silently, and SwiftData's runtime `ModelContainer.load` path catches the resulting "your bundle claims iCloud + AppGroup but I can't verify those" condition as `loadIssueModelContainer`. The error is opaque (no human-readable explanation, just enum case + generic NSError) — easy to misdiagnose as a SwiftData bug or a model-schema regression.

**Fix.**

1. **Sequence matters.** The round-26 activation checklist (`docs/round26-activation-checklist.md` Step 1 → Step 2 → Step 3) names this explicitly: portal resources first (App IDs + App Group + iCloud Container), THEN team switch, THEN entitlements wiring. Skipping straight to wiring is the failure mode.

2. **If wiring lands prematurely, roll back the `CODE_SIGN_ENTITLEMENTS:` lines and `xcodegen generate` again.** Tests immediately return to green. The `.entitlements` files themselves can stay — they're inert until referenced from `CODE_SIGN_ENTITLEMENTS`.

3. **Don't try to "make the test container ignore entitlements."** SwiftData reads the *bundle*'s entitlements, not the container's flags. There's no in-test escape hatch short of running tests in a fresh bundle without the entitlements — which is what rollback achieves anyway.

4. **Re-add the wiring AFTER the portal-side resources exist.** Workflow: open developer.apple.com/account/resources → create the App IDs + App Group + iCloud Container per `docs/round26-activation-checklist.md` → run `./scripts/check-tests.sh` (still green at this point because no wiring change yet) → re-add `CODE_SIGN_ENTITLEMENTS:` per target in `App/project.yml` → `xcodegen generate` → `./scripts/check-tests.sh` again. Now SwiftData's runtime check passes because the identifiers actually exist on the portal.

**Guard.** No automated regression test possible at the simulator level — the failure mode exists only when entitlements claim portal resources that don't yet exist, and SwiftData treats that as a fatal init error specifically because it CAN'T reach the portal from a sandbox. Process guard: `App/project.yml` carries inline comments above each target's commented-out `CODE_SIGN_ENTITLEMENTS:` line pointing at this lesson and at the activation checklist; treat those comments as load-bearing until the portal step is done. Future tooling option: `scripts/check-entitlements-portal-state.sh` could query developer.apple.com via App Store Connect API to verify the App Group + iCloud Container exist before allowing the wiring commit, but that's gated on the user adding API key access.

**Where it was caught.** 2026-05-06, round 38a. User confirmed Apple Developer Program ACTIVE earlier in the session; combo K → A → E → G → L kicked off; A07-A08 wired entitlements in `project.yml`, regenerated xcodeproj, ran `check-tests.sh`, and got 13 SwiftDataError failures in TripDetailViewModel tests. Rollback restored green in one commit. Root-cause diagnosis took ~5 minutes via xcresulttool — the `loadIssueModelContainer` enum case was the smoking gun once isolated. Cost: 0 commits to main (rollback happened in working tree before commit), ~10 minutes of test cycles.

**Interaction with other lessons.** Sibling to L001 (ModelContainer lifecycle) and L009 (local-pass insufficient): both lessons highlight that SwiftData + Apple-platform interactions surface failures only at the integration boundary, not at the type-system level. L013 specifically extends the "the .entitlements file is data, not just decoration" insight: declaring an entitlement is a runtime contract, and the contract is enforced even in simulator + even for in-memory stores. The fix isn't more lenient SwiftData configuration; it's matching the declared entitlements to actually-existent portal resources.
