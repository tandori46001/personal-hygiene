...
## 30. Rounds 14-16 — trip carbon + emergency contacts + dose history + housekeeping icons + focus preview

### Round 14 — infrastructure-heavy (`226bf35`)

`TripCarbonEstimate` is a pure helper: haversine distance + `0.255 kg CO₂ per passenger·km` (industry-average economy-class factor) → round-trip kg. The factor is documented inline so a future user can swap it for a more authoritative source. `TripEmergencyContact` is a value type stored JSON-encoded on `Trip.emergencyContactsJSON` (avoids another schema migration); `TripEmergencyContactsSection` renders the list + an inline add row with phone-pad input. `QuietHoursStore` is the recurring counterpart of `PauseNotificationsStore` (round 12) — wraps around midnight (defaults 22:00 → 07:00). `PendingNotificationsGroup.grouped(_:)` returns `[(category, [identifier])]` in canonical `Category.allCases` order so DiagnosticsView can render disclosure-per-category. `TemplatePresetSeeds` enum holds `morningRoutine` / `workday` / `weekendChores` block bundles (each is a `[BlockSeed]`). `HydrationWeeklyAverage.averageMilliliters(dailyTotals:)` is a pure helper for the round-15 caption — rounds to nearest 10ml. `Trip.expensesJSON` carries `[TripExpense]`; `TripExpensesSection` shows a per-currency total summary footer when ≥2 expenses present.

### Round 15 — deferred UI close-out (`fd88a46`)

`MilestoneDefaultBundle.standard` is a `[Seed]` with the canonical 6m / 3m / 1m / 1w trip-prep set; `TripDetailViewModel.addStandardMilestoneBundle()` is idempotent against existing `daysBefore` so the user can tap the empty-state button safely without double-creating. `TemplateDurationCalculator` is a 2-method helper (`totalMinutes(_:)` + `formatted(_:)`); the TemplateEditor footer uses both to render `Total: Xh Ym · N blocks`. `MedicationDoseHistory.recent(...)` aggregates `BlockCompletion` records into a 30-day medication-only newest-first list (filters via `Block.medicationConceptIdentifier != nil`). `HydrationDashboardView` got the weekly-average caption ("Weekly average: X ml/day") below the chart, gated on `weeklyAvg > 0`. Birthdays gained a relationship-tag filter chip row backed by `BirthdayRelationshipStore` (round 12).

### Round 16 — medication + carbon + housekeeping + focus (`61c0a1a`)

`DoseHistoryView` is the round-16 surface for `MedicationDoseHistory` — read-only list with text-selectable concept identifier (truncate-middle for long IDs), date+time, empty state. `TripCarbonSection` reads `TripDetailViewModel.roundTripCO2Kg(home:)` and renders inline on TripDetail when both endpoints are geocoded; missing-data path is silent (no UI). `HousekeepingRoomIcons` is an 8-symbol palette + `IconChoice` value type; `HousekeepingRoomIconStore` is a UserDefaults JSON dict keyed on room name (no UI wires the picker yet). `FocusFilterPreview.preview(at:in:scheduledWindows:calendar:)` returns `(activeBlock?, silencedBlocks)` for the current moment — match active window back to a deep-focus block by `blockTitle` (the only field in `DeepFocusFilter.FocusWindow`); silenced blocks are non-deep-focus blocks whose start time falls inside the active window. `Result` dropped `Sendable` because `Block` (SwiftData @Model) isn't Sendable.

---

## 31. Round 17 — deferred-UI close-out (7 wires, no new infra)

Round 17 is wires-only: every store / helper added in rounds 13-16 that didn't yet have a UI surface gets one. No new domain models, no new services, no new lessons. The shape:

1. **DoseHistoryView NavigationLink.** `MedicationComplianceView` gains a "Dose history" row beneath the 7-day compliance section. Loader = `MedicationComplianceViewModel.doseHistory(days:now:)` which calls a new `RoutineRepository.recentCompletions(days:now:calendar:)` that returns `[BlockCompletion]` over the trailing window newest-first, then runs them through `MedicationDoseHistory.recent(...)` (round 15) for medication-only filtering.
2. **TemplatePresets insertion menu.** `TemplateEditorView` blocks section gains a Menu listing every `TemplatePresetSeeds.Preset.allCases` entry. Tapping inserts that preset's `[BlockSeed]` via `TemplateEditorViewModel.insertPreset(_:)` — every seed is appended after the last existing block, with a single shared offset that preserves seed-to-seed spacing. `baseOffset = max(0, lastBlockEnd - firstSeedStart)` so presets that already start after the last block don't shift; presets that would overlap shift to fit at the tail.
3. **FocusFilterPreview UI surface.** `FocusScheduleView` gains an optional `blocksProvider: () -> [Block]` parameter (default `{ [] }` for callers without access). When non-empty, a "Right now" section runs `FocusFilterPreview.preview(...)` and renders the active block + silenced count. `SettingsView` plumbs the closure from a new `routineRepository: (any RoutineRepository)?` parameter; `ContentView` passes `env.routineRepository`. Inert when the provider is empty so the existing FocusScheduleView callers (previews, tests) keep working.
4. **Quiet hours UI.** New `quietHoursSection` in `SettingsViewRound17.swift` extension: toggle, plus start/end `DatePicker`s shown only when enabled. Bound directly to `QuietHoursStore` — no view-model layer. Footer reminds the user medications keep firing.
5. **BackupAutoFrequency picker.** New `backupAutoFrequencySection` (off / weekly / daily) — a wrapped `Picker` over `BackupAutoFrequencyStore.Frequency.allCases`. Footer notes the auto-backup engine ships in a future phase; this captures intent only.
6. **PendingNotificationsGroup disclosure.** New `DiagnosticsViewRound17Sections.swift` adds `pendingByGroupSection` — a top-level `DisclosureGroup` inside the existing Advanced disclosure. Re-classifies the already-loaded `pendingDetails` list (no new closure on `DiagnosticsActions`) by feeding the identifiers through `PendingNotificationsGroup.grouped(_:)`. Each category opens a sub-disclosure with the actual identifiers (truncate-middle for long IDs).
7. **HousekeepingRoomIcons picker.** `HousekeepingListView` new-task sheet gains a `roomIconPickerSection` that appears only when a room has been chosen. Picker rows are `HousekeepingRoomIcons.palette` IconChoices (label + SF Symbol). On confirm, the selected `iconID` is persisted via `HousekeepingRoomIconStore.setIconID(_:forRoom:)`. Existing `HousekeepingRow` renders the stored SF Symbol next to the room name when present.

**Test additions (5):**
- `SwiftDataRoutineRepositoryTests.test_recentCompletions_returnsRollingWindowNewestFirst` — pins the new repository method's window + ordering contract.
- `TemplateEditorViewModelTests.test_insertPreset_appendsSeedsShiftedAfterLastBlock` — confirms relative seed spacing is preserved when no shift is needed.
- `TemplateEditorViewModelTests.test_insertPreset_shiftsSeedsWhenTemplateExtendsPastFirstSeed` — confirms the `(lastEnd - firstSeed)` shift applies when the template extends past the preset's first seed.
